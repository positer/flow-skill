...
class FlowDialogBridge:
    ...
    def current_chat_step(self) -> dict:
        """Return the current step with stable conversation rendering fields."""
        return render_chat_step(self._current_step)
    ...
    def history(self) -> list[dict]:
        """Return chat-renderable steps emitted so far."""
    ...
    def _record_current(self):
        if self._current_step:
            self._history.append(render_chat_step(self._current_step))


def render_chat_step(step: dict | None) -> dict:
    """Normalize a workflow step for visible chat-area rendering.

    Host agents should show ``chat_header`` and ``rendered_prompt`` or
    ``message`` in the conversation area before executing or advancing.
    """
    if not step:
        return {"type": "empty", "chat_header": "Flow", "message": "No active step."}
    normalized = dict(step)
    typ = normalized.get("type", "unknown")
    cid = normalized.get("component_id", normalized.get("id", "?"))
    if typ == "dialog":
        mode = normalized.get("mode", "Build")
        normalized.setdefault("chat_header", f"Flow {cid} {mode}")
        normalized.setdefault("display_title", f"Flow {cid} {mode}")
        normalized.setdefault("rendered_prompt", normalized.get("prompt", ""))
        normalized.setdefault("requires_conversation", True)
    elif typ == "logic":
        normalized.setdefault("chat_header", f"Flow {cid} Logic")
        normalized.setdefault("display_title", f"Flow {cid} Logic")
        normalized.setdefault("rendered_prompt", normalized.get("prompt", ""))
        normalized.setdefault("requires_conversation", True)
    elif typ == "branch":
        branch = str(normalized.get("branch", "")).upper()
        target = normalized.get("target")
        normalized.setdefault("chat_header", f"Flow {cid} Logic Decision")
        normalized.setdefault("display_title", f"Flow {cid} Logic -> {branch}")
        normalized.setdefault("message", f"Logic evaluated {branch}; goto component {target}.")
        normalized.setdefault("requires_conversation", True)
    elif typ in ("complete", "error"):
        normalized.setdefault("chat_header", "Flow Complete" if typ == "complete" else "Flow Error")
        normalized.setdefault("display_title", normalized["chat_header"])
        normalized.setdefault("requires_conversation", True)
    return normalized
...
def ensure_flow_capability(flow_dir: str = "Flow") -> dict | None:
    ...
    tools = {
        ...
        "render_chat_step": render_chat_step,
    }
