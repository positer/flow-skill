...
| `prompt` | string | yes | Base prompt. Use `{#prompt#}` as placeholder for user input. `base_prompt` may be accepted as a read-only alias by tools, but files should save `prompt`. |
...
- **Goal mode**: runs as a Build-style step followed by an injected Logic verification step; FALSE loops back to the Goal component, TRUE continues
...
## Dialogue-Area Agent Protocol

Agent integrations MUST use `run_workflow_iter()` or `FlowDialogPlugin.FlowDialogBridge`
as the canonical runtime. CLI stdout is a debug/fallback interface only.

Each yielded step includes chat-rendering metadata such as:
...
- `chat_header`
- `rendered_prompt`
- `goto_true` / `goto_false` for logic
- `branch` and `target` for branch-decision steps
- `message` for completion, branch, and error states

Required rendering behavior:

1. Dialog step: show `Flow {id} {mode}` in the conversation area, show `rendered_prompt`, then execute the prompt in the requested mode.
2. Logic step: show `Flow {id} Logic`, show the condition, evaluate whether it is already implemented in the workspace, then submit TRUE or FALSE.
3. Branch step: show the TRUE/FALSE decision and goto target before continuing.
4. Completion/error: show the final state in the conversation area.

If the host agent cannot switch modes programmatically, it should keep the mode
label visible and follow the mode's intent in the same conversation.
