...
class FlowComponent:
    def __init__(self, data: dict):
        ...
        self.prompt: str = data.get("prompt", data.get("base_prompt", ""))
...
def _flow_display_title(workflow_name: str, step_id: int, kind: str, mode: str | None = None) -> str:
    label = mode if kind == "dialog" else "Logic"
    return f"Flow {step_id} {label}"


def _chat_step_base(workflow_name: str, kind: str, step_id: int, prompt: str) -> dict:
    return {
...
def _dialog_step(workflow_name: str, component: FlowDialogComponent, prompt: str) -> dict:
    step = _chat_step_base(workflow_name, "dialog", component.id, prompt)
    step.update({
        "type": "dialog",
        "id": component.id,
        "mode": component.mode,
        "display_title": _flow_display_title(workflow_name, component.id, "dialog", component.mode),
        "chat_header": f"Flow {component.id} {component.mode}",
        "requires_conversation": True,
        "execution_hint": (
            "Present this prompt visibly in the conversation area and execute it "
            f"as a {component.mode} step. If the host cannot switch modes, keep "
            "the mode label visible and follow that mode's intent."
        ),
    })
    return step
...
def _logic_step(
    ...
    *,
    ...
    step = _chat_step_base(workflow_name, "logic", component_id, prompt)
    step.update({
        "type": "logic",
        "id": component_id,
        "goto_true": goto_true,
        "goto_false": goto_false,
        "display_title": _flow_display_title(workflow_name, component_id, "logic"),
        "chat_header": f"Flow {component_id} Logic",
        "requires_conversation": True,
        "evaluation_hint": (
            "Evaluate visibly in the conversation area whether this condition is "
            "already implemented in the current workspace. TRUE jumps to goto_true; "
            "FALSE jumps to goto_false."
        ),
        "synthetic": synthetic,
    })
    if source_component_id is not None:
        step["source_component_id"] = source_component_id
    return step


def _branch_step(workflow_name: str, component_id: int, condition: bool, target: int) -> dict:
    ...
    return {
        ...
        "branch": branch,
        "target": target,
        "display_title": f"Flow {component_id} Logic -> {branch.upper()}",
        "chat_header": f"Flow {component_id} Logic Decision",
        "message": f"Logic evaluated {branch.upper()}; goto component {target}.",
        "requires_conversation": True,
    }
...
def run_workflow_iter(name: str, flow_dir: str, user_input: str | None = None):
    ...
    if cycle_warnings:
        yield {
            ...
            "display_title": f"Flow {name} Cycle Warning",
            "chat_header": "Flow Cycle Warning",
            "message": "\n".join(cycle_warnings),
            "requires_conversation": True,
        }
    ...
    while 0 <= idx < len(components):
        ...
        if isinstance(c, FlowDialogComponent):
            prompt = _build_dialog_prompt(c, user_input)
            step = _dialog_step(wf.name, c, prompt)
            response = yield step
            if response is None:
            ...
            if c.mode == "Goal":
                ...
                if condition:
                    yield _branch_step(wf.name, cid + 10000, True, cid + 1)
                    idx += 1
                else:
                    yield _branch_step(wf.name, cid + 10000, False, cid)
            else:
                idx += 1

        elif isinstance(c, FlowLogicComponent):
            prompt = c.prompt.replace("{#prompt#}", user_input or "")
            step = _logic_step(wf.name, cid, prompt, c.goto_true, c.goto_false)
            result = yield step
            condition = result.get("condition", False) if isinstance(result, dict) else False
            ...
            if new_idx < 0:
                ...
                return
            yield _branch_step(wf.name, cid, condition, target)
            idx = new_idx
    ...
    yield {
        ...
        "display_title": f"Flow {wf.name} Complete",
        "chat_header": "Flow Complete",
        "requires_conversation": True,
    }
